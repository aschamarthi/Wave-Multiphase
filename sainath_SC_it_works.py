#!/usr/bin/env python3
"""
Sainadh Chamarthi, Caltech ..... sainath@caltech.edu
...
XMIN_V, XMAX_V = 0.0, 0.111    # 11.1 cm
YMIN_V, YMAX_V = 0.0, 0.074    # 7.4 cm
DX_V = (XMAX_V - XMIN_V) / NX
DY_V = (YMAX_V - YMIN_V) / NY

CFL_NUM  = 0.4
T_END_V  = 67e-6
T_SAVES  = [4e-6, 17e-6, 40e-6, 67e-6]
USE_EMERGENCY_REPAIR = True  # Set False for strictly conservative PP-flux-only runs. Its a pain in the neck.
...
WLIQ   = wp.constant(wp.float64(2.0))       # p_inf threshold for liquid... Not used
...
WSIGMA_SMOOTH = wp.constant(wp.float64(0.01))   # Ducros×pressure threshold
...
def mp5_ur(vm1: wp.float64, v0: wp.float64, vp1: wp.float64,
    ...
    # charstencil = v(-1:3); cs(-2)=vm1, cs(-1)=v0, cs(0)=vp1, cs(1)=vp2, cs(2)=vp3
...
# ── MUSCL κ=1/3 in primitive space ────────────────────────────────────────────
@wp.func
...
# ══════════════════════════════════════════════════════════════════════════════
# SC Eigenvector transforms  —  x-direction  (mx=1, my=0)
# ══════════════════════════════════════════════════════════════════════════════
#
# SC state:  U = [ar1, ar2, ru, rv, p, a1]
#                  u0   u1   u2  u3  u4  u5
#
# Interface averages passed in:
#   rho  = ar1a + ar2a
#   u    = (ru_L + ru_R) / (2*rho)   (normal velocity, x-dir)
#   v    = (rv_L + rv_R) / (2*rho)   (tangential velocity, x-dir)
#   c    = sound speed at interface
#   Y1   = ar1a / rho
#   Y2   = ar2a / rho
#
# L (left eigenvectors as rows):
#   Row 0 (acoustic -):  [ un/(2c),  un/(2c), -mx/(2c), -my/(2c),  1/(2c^2),  0 ]
#   Row 1 (contact m1):  [ 1,         0,        0,        0,       -Y1/c^2,    0 ]
#   Row 2 (contact m2):  [ 0,         1,        0,        0,       -Y2/c^2,    0 ]
#   Row 3 (shear):       [-ut,        -ut,      -my,       mx,       0,         0 ]
#   Row 4 (VF):          [ 0,         0,        0,        0,        0,          1 ]
#   Row 5 (acoustic +):  [-un/(2c), -un/(2c),  mx/(2c),  my/(2c),  1/(2c^2),  0 ]
#
# x-dir: mx=1, my=0  =>  un=u (normal), ut=v (tangential)
#
# R (right eigenvectors as columns):
#   Col 0 (acoustic -):  [ Y1,   Y2,   u-c,  v,    c^2,  0 ]
#   Col 1 (contact m1):  [ 1,    0,    u,    v,    0,    0 ]
#   Col 2 (contact m2):  [ 0,    1,    u,    v,    0,    0 ]
#   Col 3 (shear):       [ 0,    0,   -my,   mx,   0,    0 ]  = [0,0,0,1,0,0] x-dir
#   Col 4 (VF):          [ 0,    0,    0,    0,    0,    1 ]
#   Col 5 (acoustic +):  [ Y1,   Y2,   u+c,  v,    c^2,  0 ]
...
def to_phys_x(w: S6, c: wp.float64, rho: wp.float64,
    ...
    """
    Back-project characteristic vector to SC physical space (x-dir).
    Returns [ar1, ar2, rho*u, rho*v, p, a1].
    u_avg, v_avg = interface-averaged normal and tangential velocities.
    """
    c2   = c * c
    Y1   = ar1 / rho
    Y2   = ar2 / rho
...
def to_phys_y(w: S6, c: wp.float64, rho: wp.float64,
              ...
              u_avg: wp.float64, v_avg: wp.float64) -> S6:
    """
    Back-project characteristic vector to SC physical space (y-dir).
    Returns [ar1, ar2, rho*u, rho*v, p, a1].
    u_avg = tangential (x) velocity at interface
    v_avg = normal    (y) velocity at interface
    """
    c2   = c * c
    Y1   = ar1 / rho
    Y2   = ar2 / rho
...
# (interface-averaged state computed inline in flux kernels)
...
def ic_kernel2(ar1: wp.array2d(dtype=wp.float64),
    ...
    shock_x   = wp.float64(0.0289)
    drop_xc   = wp.float64(0.04);  drop_yc  = wp.float64(0.037)
    drop_r    = wp.float64(0.011)  # outer circle radius
    drop_core = wp.float64(0.004)  # inner air pocket radius
    
    dx2_outer = (xi - drop_xc) * (xi - drop_xc) \
              + (yj - drop_yc) * (yj - drop_yc)
    
    dx2_inner = (xi - drop_xc) * (xi - drop_xc) \
              + (yj - drop_yc) * (yj - drop_yc)
    
    if xi < shock_x:
        a1r1 = wp.float64(1.0e-8);  a2r2 = wp.float64(3.7579)
        uu   = wp.float64(574.57);  vv   = wp.float64(0.0)
        pp   = wp.float64(6.6189e5); aa  = wp.float64(1.0e-8)
    elif dx2_inner <= drop_core * drop_core:
        a1r1 = wp.float64(1.0e-8);  a2r2 = wp.float64(1.17)
        uu   = wp.float64(0.0);     vv   = wp.float64(0.0)
        pp   = wp.float64(101312.5); aa  = wp.float64(1.0e-8)
    elif dx2_outer <= drop_r * drop_r:
        a1r1 = wp.float64(1000.0);  a2r2 = wp.float64(1.0e-8)
        uu   = wp.float64(0.0);     vv   = wp.float64(0.0)
        pp   = wp.float64(101312.5); aa  = W1 - wp.float64(1.0e-8)
    else:
        a1r1 = wp.float64(1.0e-8);  a2r2 = wp.float64(1.17)
        uu   = wp.float64(0.0);     vv   = wp.float64(0.0)
        pp   = wp.float64(101312.5); aa  = wp.float64(1.0e-8)
...
# ── Ducros × pressure-smoothness sigma sensor ──────────────────────────────────
#
# sigma_x(i,j) and sigma_y(i,j) quantify the local shock strength in the
# x- and y-sweeps respectively:
#   sigma = (aa / (bb + eps)) * ducros
# where aa/bb are the 4th-order pressure difference / sum stencils and ducros
# is the compressibility-weighted vorticity sensor.
# Small sigma (<0.01) indicates a smooth flow region.
#
@wp.kernel
def ducros_kernel(u_p:     wp.array2d(dtype=wp.float64),
                  v_p:     wp.array2d(dtype=wp.float64),
                  p_p:     wp.array2d(dtype=wp.float64),
                  sigma_x: wp.array2d(dtype=wp.float64),
                  sigma_y: wp.array2d(dtype=wp.float64),
                  nxg: int, nyg: int):
    i, j = wp.tid()

    # Guard: pressure stencil requires ±2, velocity stencil ±1
    if i < 2 or i >= nxg - 2 or j < 2 or j >= nyg - 2:
        sigma_x[i, j] = wp.float64(1.0)
        sigma_y[i, j] = wp.float64(1.0)
        return

    # Central-difference velocity derivatives (un-scaled; Ducros is dimensionless)
    der_ux = wp.float64(0.5)*(-u_p[i-1, j] + u_p[i+1, j])
    der_vx = wp.float64(0.5)*(-v_p[i-1, j] + v_p[i+1, j])
    der_uy = wp.float64(0.5)*(-u_p[i, j-1] + u_p[i, j+1])
    der_vy = wp.float64(0.5)*(-v_p[i, j-1] + v_p[i, j+1])

    div_u  = der_ux + der_vy
    curl_u = der_vx - der_uy
    ducros = div_u*div_u / (div_u*div_u + curl_u*curl_u + wp.float64(1.0e-30))

    # x-direction: 4th-order difference / sum of pressure 
    pm2x = p_p[i-2, j];  pm1x = p_p[i-1, j];  p0x = p_p[i, j]
    pp1x = p_p[i+1, j];  pp2x = p_p[i+2, j]
    aa_x = wp.abs(-pm2x + wp.float64(16.0)*pm1x - wp.float64(30.0)*p0x
                  + wp.float64(16.0)*pp1x - pp2x)
    bb_x = wp.abs( pm2x + wp.float64(16.0)*pm1x + wp.float64(30.0)*p0x
                  + wp.float64(16.0)*pp1x + pp2x)
    sigma_x[i, j] = (aa_x / (bb_x + wp.float64(1.0e-30))) * ducros

    # y-direction
    pm2y = p_p[i, j-2];  pm1y = p_p[i, j-1];  p0y = p_p[i, j]
    pp1y = p_p[i, j+1];  pp2y = p_p[i, j+2]
    aa_y = wp.abs(-pm2y + wp.float64(16.0)*pm1y - wp.float64(30.0)*p0y
                  + wp.float64(16.0)*pp1y - pp2y)
    bb_y = wp.abs( pm2y + wp.float64(16.0)*pm1y + wp.float64(30.0)*p0y
                  + wp.float64(16.0)*pp1y + pp2y)
    sigma_y[i, j] = (aa_y / (bb_y + wp.float64(1.0e-30))) * ducros
...
def fx_kernel(ar1: wp.array2d(dtype=wp.float64),
              ar2: wp.array2d(dtype=wp.float64),
              ru:  wp.array2d(dtype=wp.float64),   # <-- rho*u  (was u_p)
              rv:  wp.array2d(dtype=wp.float64),   # <-- rho*v  (was v_p)
              p_p: wp.array2d(dtype=wp.float64),
              al:  wp.array2d(dtype=wp.float64),
              sigma_x: wp.array2d(dtype=wp.float64),
    ...
    # ── Load stencil: SC state [ar1, ar2, ru, rv, p, a1] ──────────────────
    pm2 = S6()
    ...
    # ── Ducros sigma: smooth-shear indicator 
    smax_x = wp.max(wp.max(sigma_x[ii-1, jj], sigma_x[ii, jj]), wp.max(sigma_x[ii+1, jj],sigma_x[ii+2, jj]))
    use_smooth_shear = smax_x < WSIGMA_SMOOTH
    ...
    u_avg = W0p5 * (p0.v2 + pp1.v2) / rhoa   # normal velocity
    v_avg = W0p5 * (p0.v3 + pp1.v3) / rhoa   # tangential velocity
    ...
    # ── Shear characteristic (v3): Central
    # Formula: -1/12*wm1 + 7/12*w0 + 7/12*w1 - 1/12*w2  
    if use_smooth_shear:
        iph_v3 = (- wp.float64(1.0/12.0)*wm1.v3
                  + wp.float64(7.0/12.0)*w0.v3
                  + wp.float64(7.0/12.0)*w1.v3
                  - wp.float64(1.0/12.0)*w2.v3)
        wL.v3 = iph_v3
        wR.v3 = iph_v3
    ...
    # ── First-order fallback (first-order char reconstruction) ───────────────
    ...
    if wp.isnan(phL.v4):                               phL.v4 = phfL.v4
    ...
    if wp.isnan(phR.v4):                               phR.v4 = phfR.v4
...
# ══════════════════════════════════════════════════════════════════════════════
# Updated gy_kernel  (same logic, y-direction)
# ══════════════════════════════════════════════════════════════════════════════
...
def gy_kernel(ar1: wp.array2d(dtype=wp.float64),
    ...
    smax_y = wp.max(wp.max(sigma_y[ii, jj-1], sigma_y[ii, jj]), wp.max(sigma_y[ii, jj+1],sigma_y[ii, jj+2]))
    use_smooth_shear = smax_y < WSIGMA_SMOOTH
    ...
    pa   = W0p5 * (p0.v4 + pp1.v4)
    rhoa = ar1a + ar2a
    u_avg = W0p5 * (p0.v2 + pp1.v2) / rhoa   # tangential (x) at y-interface
    v_avg = W0p5 * (p0.v3 + pp1.v3) / rhoa   # normal    (y) at y-interface
    ca    = sound_speed(rhoa, pa, a1a)
    gdum, pinf_a = mix_gamma_pinf(a1a)
...
def main():
    ...
    def compute_fluxes():
        # Ducros × pressure sigma (needs u_p, v_p, p_p up-to-date from update_prims)
        wp.launch(ducros_kernel, dim=(NXG, NYG),
                  inputs=[u_p, v_p, p_p, sigma_x, sigma_y, NXG, NYG],
                  device=DEVICE)
        # x-fluxes: interfaces G-1 .. G+NX-1  (NX+1 total), for all NY interior cells
        wp.launch(fx_kernel, dim=(NX+1, NY),
                  inputs=[ar1,ar2,ru,rv,p_p,al, sigma_x,
                          fx0,fx1,fx2,fx3,fx4,fx5,srcx,
                          G-1, G],    # ofs_i=G-1 (first interface), ofs_j=G
                  device=DEVICE)
        # y-fluxes
        wp.launch(gy_kernel, dim=(NX, NY+1),
                  inputs=[ar1,ar2,ru,rv,p_p,al, sigma_y,
    ...
    def limit_fluxes(dt_v):
        # Wong staged flux limiter: partial densities, volume fractions,
        # then rho*c_tilde^2. Each stage uses the flux from the previous stage.
